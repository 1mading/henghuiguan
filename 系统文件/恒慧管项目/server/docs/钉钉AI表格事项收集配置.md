...
|-------------|------|------|
| 事项标题 | 文本 | 必填 |
| 系统 | 单选/文本 | 如 NCC、其它业务系统；**仅 NCC 会发新任务消息提醒** |
| 详细说明 | 多行文本 | 可选 |
| 期望完成日期 | 日期 | 空则默认明天 |
| 优先级 | 单选：普通 / 重要 / 紧急 | 可选 |
| 提交人 | 人员 | 表单自动带出当前填写人 |
...
> 提单人可以是信息中心业务成员，也可以是财务中心等部门的「通知联系人」（人员档案中有橙色「联系人」标记即可，无需能登录恒慧管）。  
> 人员档案里提单人的 `dingTalkUserId` 须与钉钉一致（可重新同步通讯录；同步时勾选对应部门）。  
> 联系人收到的完成通知为纯文本：仅任务名称与「已完成」，无打开应用链接。

> 负责人**不要**放在表单里，由 `.env` 固定，避免填错。负责人须为业务成员（可登录、可接任务）。
...
6. 重复提交同一记录，确认不重复建任务
7. 负责人点「开始」：须填写预计工时后才进入进行中，之后计入当周饱和度
...
- **工时**：表单入库预计工时为 0；负责人开始执行/完成前须填写预计工时，之后与常规任务相同规则计入饱和度与实际工时
...
- 确认表单「系统」是否为 **NCC**（非 NCC 故意不发新任务提醒，仍会入库并出现在工作台「我的待办」）
...
- 工作台「我的待办」卡片会展示「系统 xxx」标签（有传该字段时）
