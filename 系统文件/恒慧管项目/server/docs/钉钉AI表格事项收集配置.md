...
|-------------|------|------|
| 事项标题 | 文本 | 必填 |
| 系统 | 单选/文本 | 如 NCC、其它业务系统；写入待办，**表单提单均会给负责人发新任务消息提醒** |
| 详细说明 | 多行文本 | 可选 |
| 期望完成日期 | 日期 | 空则默认明天 |
| 优先级 | 单选：普通 / 重要 / 紧急 | 可选 |
| 提交人 | 人员 | 表单自动带出当前填写人；用作恒慧管「提单人」 |
| 附件 | 附件 | 可选，支持图片/文档等（单文件 ≤20MB） |

> **提交人变量注意**：自动化里请插入「提交人」的 **姓名**（`{{提交人姓名}}`），不要插 **userid**。  
...
3. **执行动作**：`HTTP 请求`
   ...
   - 请求体（**所有文本字段变量两侧必须加英文双引号**；附件见下方两种方式，二选一）：

**方式 A（推荐，单附件最稳）：整段插入「附件内容」对象**
...
> 在变量树中选：**附件 → 附件内容**（整项，不是下面 4 个子字段），插入到 `"附件内容":` 后面，**不要加引号**。  
> 钉钉会展开为合法 JSON 对象（内含附件名、临时链接等数组），单文件、多文件都适用。

**方式 B（原四字段并行，仅当方式 A 不可选时用）**
...
> 四个附件子字段 **不要加引号**（须展开为 JSON 数组）。若只有 1 个文件时自动化报「格式/特殊字符」错误，请改用 **方式 A**。

**方式 C（钉钉 JSON 编辑器会自动给变量加引号时用）**

钉钉在 JSON 模式下点选插入变量时，**常会自动加上双引号**。此时子字段「附件名」展开为 `{...}` 对象，被引号包住就会报 **JSON 格式错误**（运行日志里常见 `"附件名": "{"`）。

请改用 **整段附件内容（方式 A）**，或 **只传链接/文件名字符串（可带引号）**：
...
```json
{
  ...
  "submitterName": "{{提交人姓名}}",
  "submittedAt": "{{记录创建时间}}",
  "attachmentUrl": "{{附件.附件内容.附件临时链接}}",
  "attachmentName": "{{附件.附件内容.附件名}}"
}
```

> 这里 **attachmentUrl / attachmentName 故意保留引号**，因为钉钉会自动加。只传链接通常能过 JSON 校验；若仍失败，务必改用方式 A 的 `"附件内容": {{附件.附件内容}}`（整项、值不加引号）。

**常见踩坑（会导致 HTTP 节点在钉钉侧失败，恒慧管收不到）**

| 错误写法 | 正确写法 |
|----------|----------|
| `"submitterName": {{提交人}}`（userid 对象） | `"submitterName": "{{提交人姓名}}"` |
| `"title": {{事项标题}}`（无引号） | `"title": "{{事项标题}}"` |
| `"desc": {{详细说明}}` 且说明为空时 JSON 断裂 | `"desc": "{{详细说明}}"`（空也会变成 `""`） |
| 单文件时 `"附件名": image.png` | 改用方式 A，或确保变量展开为 `["image.png"]` |
| `dueDate` 插「期望完成日期」但表头是「期望完成时间」 | `"dueDate": "{{期望完成时间}}"` |
...
- 检查负责人姓名是否在人员档案中
- **节点在钉钉侧失败、恒慧管无 `[intake]` 日志**：HTTP 请求体 **JSON 不合法**（请求未发出）。按上表检查：
  - 文本字段（标题、说明、提交人、日期等）变量 **两侧必须有英文双引号**
  - `submitterName` 必须用 **提交人姓名**，不要用 userid / 提交人对象
  - 附件优先用 **整段 `{{附件.附件内容}}`**（方式 A）；四字段并行在 **单附件** 时易触发钉钉报「特殊字符」错误
- 无附件的提交：`"附件内容": {}` 或省略附件字段均可

**Q：任务创建了但没收到钉钉通知？**

- 确认负责人已绑定 `dingTalkUserId`
- 确认钉钉应用密钥已配置；未配置时仅写入应用内通知（小喇叭）
- 需重启后端后新提单才会按新规则推送

**Q：同事提交后我看不到任务？**

- 临时任务仅负责人/创建人/相关人可见；确认 `ASSIGNEE_NAME` 是你本人
- 工作台「我的待办」卡片会展示提单人（以及「表单提报」标签）
