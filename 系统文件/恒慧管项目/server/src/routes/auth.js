...
async function loginWithDingTalkUserId(dingTalkUserId) {
  ...
  // 历史 contact 已在加载/同步时迁为 member；此处兜底升格，允许登录
...
router.post('/dingtalk/auth/login-by-userid', async (req, res) => {
  ...
  // 分享演示包（SHARE_DEMO=1）允许本机预演免密登录，不依赖正式钉钉密钥
  if (!config.allowDemoLogin && !config.dingtalk.appSecret && process.env.SHARE_DEMO !== '1') {
