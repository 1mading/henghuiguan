...
const { isPerformanceAdmin } = require('../services/performance');
const { emitChange } = require('../services/realtime');

const router = express.Router();

function emitFileEntityChange(req, entityType, entityId, type) {
  emitChange({
    type,
    entityType,
...
function countStoredFileReferences(store, fileId) {
  ...
  for (const assessment of store.performanceAssessments || []) {
    for (const row of Object.values(assessment.rows || {})) {
...
function attachWikiDocToEntity(store, req, entityType, entityId, node, docUrl) {
  if (entityType === 'project') {
    ...
    persistStore();
    emitFileEntityChange(req, 'project', entityId, 'project.documents');
  ...
  try {
    ...
    if (entityType === 'task') {
      ...
      if (!isCommentImage) {
        ...
        persistStore();
        // 留言图待发送前不广播；正式留言走 data.sync → task.comments
      }
...
router.get('/files/:fileId', requireAuth, (req, res) => {
  ...
  const users = getAllUsers();
  if (found.entityType === 'performance') {
    if (!isPerformanceAdmin(req.user)) {
      return res.status(403).json({ success: false, message: '无权访问' });
    }
  } else if (found.entityType === 'project') {
...
router.delete('/files/:fileId', requireAuth, (req, res) => {
  ...
  persistStore();
  const delType = found.entityType === 'project' ? 'project.documents' : 'task.attachments';
  const delId = found.entityType === 'project' ? found.project.id : found.task.id;
  emitFileEntityChange(req, found.entityType, delId, delType);
