...
app.use((_req, res, next) => {
  ...
  // 钉钉工作台/PC 端常以 iframe 嵌入 H5；SAMEORIGIN 会导致钉钉内白屏
  res.setHeader(
    'Content-Security-Policy',
    "frame-ancestors 'self' https://*.dingtalk.com https://*.dingtalkapps.com https://*.laiwang.com",
  );
...
if (config.staticDir) {
  ...
  const petPath = path.join(config.staticDir, '桌宠演示.html');
