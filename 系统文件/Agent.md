...
项目代码与资源均在 **`系统文件/恒慧管项目/`** 下；`系统文件/` 为 Git 仓库根目录，仅保留本文件体系与必要的 Git/Cursor 工具目录。纯文档放在工作区 **`文件/`**。
...
- **接口文档同步**：新增/变更/下线 HTTP 接口（含字段语义）时，同一次交付内更新 `server/docs/第三方写入接口.md`、`workbuddy.md` 等对应文档，并同步 `getCatalog()`；细则见 `.cursor/rules/api-docs-sync.mdc`
