 ...
 * 用法: node 恒慧管项目/scripts/check-secrets.js [--staged]
 */
...
const BLOCKED_PATHS = [
  new RegExp(`^${P}/server/\\.env$`),
  new RegExp(`^${P}/server/\\.env\\.`),
  /^\.env$/,
  new RegExp(`^${P}/server/miniapp/config\\.js$`),
  new RegExp(`^${P}/server/data/`),
  new RegExp(`^${P}/server/src/db/seed-data\\.json$`),
  new RegExp(`^${P}/server/src/db/seed-data\\.local\\.json$`),
  /\.pem$/,
  /\.key$/,
  /\.p12$/,
];
...
const ALLOWED_ENV_TEMPLATES = [
  new RegExp(`^${P}/server/\\.env\\.example$`),
];
...
function main() {
  ...
  if (allErrors.length) {
    ...
    console.error(`\n允许提交的仅为模板/示例: ${P}/server/.env.example、${P}/server/miniapp/config.js.example、seed-data.example.json\n`);
    process.exit(1);
  }
